...
res.send("The user" + (' ') + (req.body.firstName) + " Has been added!" + os.EOL);
});
...
router.put("/:email", (req, res) => {
    ...
    if (friend) {  // Check if friend exists
        ...
        res.send(`Friend with the email ${email} updated.` + os.EOL);
    } else {
        // Respond if friend with specified email is not found
        res.send("Unable to find friend!" + os.EOL);
    }
});
...
router.delete("/:email", (req, res) => {
  ...
  res.send(`Friend with the email ${email} deleted.` + os.EOL);  
});
